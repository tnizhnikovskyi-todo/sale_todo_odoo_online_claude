    ...
    for b in blocks:
        ...
        for q in b.get('питання', []):
            ...
            for a in ans:
                ...
                # `say` — те саме обмеження, але словами КЛІЄНТА: воно друкується в
                # пропозиції розділом «Про що домовилися». `note` туди піти не може —
                # це внутрішня мова («сказати вголос», «звірити з керівником», рівні).
                if a.get('r') == 'warn' and not a.get('say'):
                    errs.append('%s: відповідь «з обмеженням» без формулювання `say` — '
                                'обмеження нізвідки взяти в текст пропозиції' % qid)
                if a.get('say'):
                    if has_stop(a['say']):
                        errs.append('%s: say обіцяє те, що поза периметром: «%s»'
                                    % (qid, a['say']))
                    низ = a['say'].lower()
                    for w in ['рівень', 'рівня', 'рівні', 'прайс', 'маржа', '€',
                              'дискваліф', 'стоп', 'наш контур', 'сейл', 'керівник']:
                        if w in низ:
                            errs.append('%s: у `say` внутрішня лексика «%s» — цей текст '
                                        'читає Замовник у пропозиції' % (qid, w))
                    if a['say'].strip()[:1].islower():
                        errs.append('%s: `say` друкується окремим пунктом переліку — '
                                    'починати з великої літери' % qid)
...
def фіксований_текст(html):
    ...
    рядки = рядки_блоку(html, 'KP_TERMS')
    # рядок про невичерпність переліку живе окремою константою (його місце в
    # розділі — після меж позицій), але клієнтові він їде так само
    # Два рядки клієнтського тексту живуть окремими константами (їх місце в
    # документі визначає код, а не порядок у масиві) — сторожі мусять бачити й їх
    for ім in ('МЕЖА_ПАКЕТА', 'НЕ_ВИЧЕРПНО'):
        m = re.search(r"var %s = '((?:[^'\\\\]|\\\\.)*)';" % ім, html)
        if m:
            рядки = рядки + [m.group(1)]
...
def межі_залежності(groups):
    """Які межі знімаються, якщо потрібну позицію купили.

    Половина рядків «не входить» — це не межа контуру, а **вказівник на іншу
    позицію прайсу**: «бланки вашого зразка — позиція «Друковані форми»»,
    «набори з кількох товарів — потребують позиції «Виробництво»». Коли ця
    позиція в наборі, рядок стає неправдою: КП пише «не входить» про те, що
    клієнт щойно купив. На повному наборі таких рядків десять.

    Тут вони розвʼязуються **на збірці**, а не руками: назва в лапках → id
    позиції, «рівня N» → мінімальний рівень. Руками означало б четверту копію
    того самого знання (прайс, журнал, рецепти — і тут), яка розійшлася б при
    першому перейменуванні.

    Назви в тексті стоять у відмінках і скорочено («Управлінським обліком»,
    «Обслуговуванні», «Міграція»), тому після точного збігу пробується
    **спільний префікс** — не менше шести літер і однозначний. Лапки, що не
    вказують на позицію («по всій групі», «Гривня and … Kopiyka»), просто не
    розвʼязуються; якщо ж поруч стоїть слово «позиці», а назва не знайшлася —
    це попередження, бо схоже на описку в назві.
    """
    назви = {}
    for gr in groups:
        for it in gr.get('items', []):
            назви[it['n']] = it['id']
    warns, карта, скільки = [], {}, 0

    def розвʼязати(ім):
        if ім in назви:
            return назви[ім]
        кандидати = []
        for n, iid in назви.items():
            спільне = 0
            for a, b in zip(ім.lower(), n.lower()):
                if a != b:
                    break
                спільне += 1
            if спільне >= 6:
                кандидати.append((спільне, iid))
        кандидати.sort(reverse=True)
        if len(кандидати) == 1 or (len(кандидати) > 1 and кандидати[0][0] > кандидати[1][0]):
            return кандидати[0][1]
        return None

    for gr in groups:
        for it in gr.get('items', []):
            рядки = []
            for b in it.get('bounds', []) or []:
                знято = []
                for m in re.finditer('«([^»]+)»', b):
                    iid = розвʼязати(m.group(1))
                    if not iid or iid == it['id']:
                        if not iid and re.search('позиці', b[max(0, m.start() - 40):m.start()]):
                            warns.append('%s: у «не входить» названо «%s», але позиції з такою '
                                         'назвою в прайсі немає — описка?' % (it['id'], m.group(1)))
                        continue
                    хвіст = b[m.end():m.end() + 25]
                    рів = re.search(r'рівня (\d)', хвіст)
                    знято.append({'on': iid, 'lv': int(рів.group(1)) if рів else 1})
                рядки.append(знято)
                скільки += 1 if знято else 0
            if any(рядки):
                карта[it['id']] = рядки
...
def main():
    ...
    print('    меж, що знімаються купленою позицією: %d — у КП і в «сказати відразу» вони не друкуються' % скільки_меж)
