...
7. **Периметр.** Жоден крок не ПИШЕ туди, чого ми не продаємо: cron, нова модель,
   план рахунків, ставки податків, зарплатні моделі; сервер-дія зі state «code»
   (Python) або «webhook» (послуга поза скоупом); власне поле з «compute».
   Читання цих моделей дозволене й потрібне — перевірити, що штатний cron Odoo
   живий, це правильна робота.
...
def тільки_перевіряє(rec):
    """Рецепт, який нічого не пише в базу і нікому нічого не передає.

    Знайдено 10.09.2026 звітом «кроки проти годин»: позиція «Склад» показала
    48 годин при 39 кроках і всього 4 кроках-послугах — менше за будь-яку
    сусідню. Причина виявилась не в ціні, а в рецептах: чотири з пʼяти пунктів
    рівня 1 складалися з єдиного «перевірити, що модель у базі є».

    Це та сама поломка, що згенерована «штатна поведінка» замість конфігурації,
    тільки з іншого боку: **пункт продано, і робота в ньому є навіть тоді, коли
    записувати нічого** — показати процес на живих документах і домовитись про
    порядок. Рецепт без цього кроку робить план повним на вигляд, а клієнта
    лишає без здачі.

    Таких рецептів було 18 у семи позиціях — усі написані до появи шаблону
    «штатна_поведінка», який саме тому й закінчується кроком-послугою.

    Попередження, а не помилка: рецепт може складатися з самих перевірок
    законно — якщо все, що продано в пункті, робить сусідній пункт, а тут
    лишається тільки переконатися.
    """
    tpl = rec.get('шаблони') or {}
    ПИШЕ = {'створити', 'записати', 'параметр', 'властивість'}

    def дії(кроки, глибина=0):
        out = []
        if глибина > 4:
            return out
        for st in кроки:
            ...
            d = st.get('дія')
            if d == 'шаблон':
                t = tpl.get(st.get('назва')) or {}
                out += дії(t.get('кроки') or [], глибина + 1)
            elif d == 'для_кожного':
                out += дії([st.get('крок') or {}], глибина + 1)
            else:
                out.append(d)
        return out
    ...
    for pid, lvs in rec['позиції'].items():
        for lk, items in lvs.items():
            for txt, body in items.items():
                d = set(дії(body.get('кроки') or []))
                if not (d & ПИШЕ) and 'послуга' not in d:
                    warns.append('%s р.%s «%s»: рецепт тільки перевіряє — нічого '
                                 'не пише в базу й нікому нічого не передає. '
                                 'Пункт продано: або тут є що налаштувати, або '
                                 'потрібен крок-послуга «показати й домовитись»'
                                 % (pid, lk, txt[:40]))
    return warns
...
# ПЕРИМЕТР. Прайс перевіряється на стоп-слова («доопрацювання», «лише в XML»),
# а рецепти — ні. Тобто обіцянку клієнту ми стережемо, а план, який ту обіцянку
# ВИКОНУЄ, могли б завтра почати виконувати чужими руками, і ніхто б не сказав
# ні слова. Ця перевірка закриває розрив із того боку: не «що ми обіцяли», а
# «що план справді запише в базу клієнта».
#
# Знайдено 10.09.2026 при звірці полів: у плані є `ir.cron` і `ir.module.module`.
# Виявилося — тільки «знайти» й «перевірити», тобто читання: чи активне ШТАТНЕ
# розписання Odoo, чи стоїть модуль. Периметр цілий. Але цілий він випадково:
# додати туди «створити» ніщо не заважало.
#
# Читання не обмежуємо НАВМИСНО: перевірити, що штатний cron живий, — це і є
# правильна робота. Забороняється саме запис.
ПОЗА_ПЕРИМЕТРОМ = {
    'ir.cron': 'cron у базі клієнта — ніколи (CLAUDE.md, межі)',
    'ir.model': 'нова модель = кастомний модуль',
    'ir.module.module': 'модулі ставляться застосунками, а не записом у таблицю',
    'account.account': 'план рахунків поза контуром (рішення 05.09.2026)',
    'account.tax': 'ставки податків поза контуром',
    'account.tax.group': 'ставки податків поза контуром',
    'account.fiscal.position': 'податкові правила поза контуром',
    'hr.contract': 'кадровий облік поза контуром',
    'hr.payslip': 'розрахунок зарплати поза контуром',
    'hr.salary.rule': 'розрахунок зарплати поза контуром',
}
# Значення, які виводять за периметр не моделлю, а вмістом. Перелік узятий
# з ir.model.fields.selection живої бази (field_id=366), а не з голови:
#   • code — «Execute Code», тобто Python у базі клієнта;
#   • webhook — «Send Webhook Notification»; вебхуки клієнту прямо названі
#     послугою ПОЗА скоупом, і саме цього значення я сам би не згадав.
СЕРВЕРНІ = ('ir.actions.server', 'base.automation')
СТАНИ_ПОЗА = {'code': 'Python у базі клієнта',
              'webhook': 'вебхуки — послуга поза скоупом'}
# Ключі поля, які роблять його обчислюваним, тобто знову ж Python.
ПОЛЕ_ПОЗА = {'compute': 'обчислюване поле = Python (Studio не має Compute)',
...
def периметр(rec):
    ...
    def кроки(steps, де):
        for s in steps:
            if not isinstance(s, dict):
                continue
            дія, модель = s.get('дія'), s.get('модель')
            if дія in CHANGING and модель in ПОЗА_ПЕРИМЕТРОМ:
                errs.append('%s: «%s» пише в %s — %s'
                            % (де, дія, модель, ПОЗА_ПЕРИМЕТРОМ[модель]))
            if дія in CHANGING:
                зн = s.get('значення') or {}
                if isinstance(зн, dict):
                    if модель in СЕРВЕРНІ and зн.get('state') in СТАНИ_ПОЗА:
                        errs.append('%s: сервер-дія зі state=«%s» — %s'
                                    % (де, зн['state'], СТАНИ_ПОЗА[зн['state']]))
                    if модель == 'ir.model.fields':
                        for k in ПОЛЕ_ПОЗА:
                            if k in зн:
                                errs.append('%s: власне поле з «%s» — %s'
                                            % (де, k, ПОЛЕ_ПОЗА[k]))
            if дія == 'для_кожного':
                кроки(s.get('кроки') or [], де)
