...
(async () => {
  ...
  // Форма, а не наявність: до 10.09.2026 межі склеювались через «;» в один абзац
  // і читалися як стіна тексту. Перевіряємо, що підпис стоїть окремим рядком, а
  // під ним ідуть пункти тією самою розміткою, що «Входить».
  const рядки = kp.text.split('\n');
  const iНе = рядки.findIndex(l => l.trim() === 'Не входить:');
  const iТак = рядки.findIndex(l => l.trim() === 'Входить:');
  ok('«входить» і «не входить» — два переліки з однаковою розміткою',
     iТак >= 0 && iНе > iТак && /^ {8}- \S/.test(рядки[iТак + 1] || '')
       && /^ {8}- \S/.test(рядки[iНе + 1] || '')
       && !рядки.some(l => /^\s*(Не )?[Вв]ходить:.+/.test(l)),
     JSON.stringify([рядки[iТак], рядки[iТак + 1], рядки[iНе], рядки[iНе + 1]]));
