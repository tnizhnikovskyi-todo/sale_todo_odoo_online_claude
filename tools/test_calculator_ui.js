...
(async () => {
  ...
  // Примітка про межу пакета стоїть ПЕРЕД групами — це перше, що читає Замовник
  // після шапки, і саме вона знімає рух «приберіть цей пункт і зменшіть ціну».
  // Перевіряємо і місце, і те, що вона не стала заголовком (жирним) у форматі.
  const рядкиКП = kp.text.split('\n');
  const iМежа = рядкиКП.findIndex(l => /^Про склад робіт: перелічені пункти — це межа пакета/.test(l));
  const iГрупа = рядкиКП.findIndex(l => /^[А-ЯІЇЄҐ]{3}/.test(l) && l === l.toUpperCase());
  ok('примітка про межу пакета стоїть до першої групи й не жирна',
     iМежа > 0 && iГрупа > iМежа && !kp.жирні.some(t => /межа пакета/.test(t))
       && /прибрати позицію повністю або взяти нижчий рівень/.test(kp.text),
     'примітка в рядку ' + iМежа + ', перша група ' + iГрупа);
  ...
  // справжня кнопка ПІД перемикачем рівня: там, куди сейл і так дивиться, коли
  // обирає рівень. Перевіряємо і вигляд (розмір, компактність, центрування в
