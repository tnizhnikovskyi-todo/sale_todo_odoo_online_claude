...
# Рівні повноти демо. Вибирає МЕНЕДЖЕР перед збіркою, і вибір змінює дві речі:
# скільки питають у клієнта і скільки заводять демо-записів. Сенс рівнів у тому,
# щоб не збирати того, чого на демо ніхто не покаже: кроки, для яких у профілі
# немає даних, на нижчих рівнях пропускаються — але ЗАВЖДИ названі в плані.
ПОВНОТА = ('зібране', 'уточнене', 'максимальне')
# Демо-записи на позицію — робоча гіпотеза до першого демо, а не заміряне число:
# правиться полем «записів_на_позицію» у профілі.
ТИПОВО_ЗАПИСІВ = {'зібране': 0, 'уточнене': 3, 'максимальне': 10}


def демо_налаштування(prof, items=None):
    """Читає блок «демо» профілю. Немає блоку — поводимось як раніше: строго."""
    d = prof.get('демо') or {}
    повнота = d.get('повнота') or 'максимальне'
    if повнота not in ПОВНОТА:
        raise SystemExit('демо.повнота = «%s» — має бути одне з: %s'
                         % (повнота, ', '.join(ПОВНОТА)))
    показуємо = d.get('показуємо') or []
    if items is not None:
        чужі = [p for p in показуємо if p not in items]
        if чужі:
            raise SystemExit('демо.показуємо називає позиції, яких немає в наборі: %s'
                             % ', '.join(чужі))
    return {'повнота': повнота,
            'показуємо': list(показуємо),
            'записів': d.get('записів_на_позицію', ТИПОВО_ЗАПИСІВ[повнота])}
...
def main(argv):
    ...
    демо = демо_налаштування(prof, bought)
    if '--всі' in argv:
        # режим покриття не має нічого пропускати: він саме про те, щоб кожен
        # рецепт хоч раз пройшов підстановку
        демо = {'повнота': 'максимальне', 'показуємо': [], 'записів': 0}
    plan, missing, no_recipe, services, пропущено = [], [], [], [], []
    for pid in order:
        if pid not in bought:
            continue
        if демо['показуємо'] and pid not in демо['показуємо']:
            continue
        lvl = int(bought[pid])
        for lk in [str(x) for x in range(1, lvl + 1)]:
            ...
            for txt, item in body.items():
                for st in item.get('кроки', []):
                    # Брак даних ловиться НА КРОЦІ, а не в кінці: інакше рівень
                    # «зібране» не міг би пропустити саме той крок, якому бракує
                    # поля, і падав би цілий план. Бракує ПОЛЯ — це голе ім'я без
                    # пробілу; усе інше (незамінена підставка, невідомий шаблон)
                    # — помилка інструмента, і вона валить збірку на будь-якому рівні.
                    свої = []
                    кроки = unroll(st, prof, свої, rec.get('шаблони'))
                    поля = sorted({m for m in свої if ' ' not in m})
                    missing += [m for m in свої if ' ' in m]
                    if поля and демо['повнота'] != 'максимальне':
                        пропущено.append((names[pid], lk, txt, поля))
                        continue
                    missing += поля
                    for step in кроки:
    ...
    print('Повнота демо: **%s** · демо-записів на позицію: %s'
          % (демо['повнота'], демо['записів']))
    if демо['показуємо']:
        print('Показуємо тільки: %s' % ', '.join(names[p] for p in демо['показуємо']))
    print()
    ...
    print()
    if пропущено:
        # Пропущене НЕ ховається: інакше демо тихо виходить біднішим, ніж думає
        # менеджер, і це видно вже перед клієнтом.
        print('## Не збираємо — немає даних у профілі')
        print()
        print('Рівень «%s» дозволяє зібрати демо без цього. На демо цих речей '
              'просто не буде.' % демо['повнота'])
        print()
        for поз, лк, txt, поля in пропущено:
            print('- %s р.%s · %s — бракує: %s' % (поз, лк, txt, ', '.join(поля)))
        print()
        if демо['повнота'] == 'уточнене':
            питання = sorted({f for _, _, _, поля in пропущено for f in поля})
            print('## Спитати в клієнта перед збіркою')
            print()
            # форма слова залежить від числа: «1 значення», «2 значення»,
            # «5 значень» — та сама пастка, що вже ловила нас у CLAUDE.md
            n = len(питання)
            форма = ('значення' if n % 10 == 1 and n % 100 != 11 else
                     'значення' if n % 10 in (2, 3, 4) and n % 100 not in (12, 13, 14)
                     else 'значень')
            print('Коротке уточнення: %d %s. Заповнити в профілі й перезапустити '
                  'план.' % (n, форма))
            print()
            for f in питання:
                де = [('%s р.%s · %s' % (p, l, t)) for p, l, t, пп in пропущено if f in пп]
                print('- `%s` — потрібне для: %s' % (f, '; '.join(де[:3])))
            print()
    if services:
    ...
    if out_json:
        io.open(out_json, 'w', encoding='utf-8').write(
            json.dumps({'клієнт': prof.get('назва'), 'кроки': plan,
                        ...
                        'повнота': демо['повнота'], 'записів': демо['записів'],
                        'не зібрано': [{'позиція': p, 'рівень': l, 'пункт': t,
                                        'бракує': f} for p, l, t, f in пропущено]},
