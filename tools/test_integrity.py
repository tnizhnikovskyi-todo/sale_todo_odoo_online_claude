...
# Частка заліку з 14.09.2026 живе в даних (`_ставки`), а сторінка бере її звідти.
# Перевірка стереже обидва способи розійтися: число, вписане назад у код, і вшитий
# блок, який відстав від даних (сторінку не перезібрали).
_html9 = io.open('artifacts/calculator.html', encoding='utf-8').read()
res.append(bc.check_diag_credit_const(_html9) == [])
print(('  ok     ' if res[-1] else '  ПРОВАЛ ')
      + 'залік: сторінка бере частку з даних, і вшите значення збігається з ними')
res.append(bc.check_diag_credit_const(
    _html9.replace("var DIAG_CREDIT = СТАВКИ['залік_діагностики'];",
                   'var DIAG_CREDIT = 1;', 1)) != [])
print(('  ok     ' if res[-1] else '  ПРОВАЛ ')
      + 'залік: число, вписане назад у код сторінки, спіймано')
res.append(bc.check_diag_credit_const(
    re.sub(r'("залік_діагностики":\s*)[\d.]+', r'\g<1>0.25', _html9, count=1)) != [])
print(('  ok     ' if res[-1] else '  ПРОВАЛ ')
      + 'залік: вшитий блок, що відстав від даних, спіймано')
...
def check_hidden_pages():
    ...
    # Рядок зі списком читається з коду, а не вписаний: 14.09.2026 до «Повного
    # переліку» додалась анкета клієнта, і отрута з вписаним текстом зламалась би
    # на кожному такому рішенні — тобто саме тоді, коли перевірка найпотрібніша.
    рядок = re.search(r"var СХОВАНІ = \[[^\]]*\];", html).group(0)
    for було, стало, слово, підпис in [
        (рядок, 'var СХОВАНІ = [];',
         'знову видима', 'сторінка повернулась у вкладки'),
        ('id="page-spec"', 'id="page-old"',
         'не сховали, а видалили', 'розмітку сторінки видалили'),
        ('id="page-form"', 'id="page-old2"',
         'не сховали, а видалили', 'розмітку анкети видалили'),
        (рядок, 'var X = [];',
