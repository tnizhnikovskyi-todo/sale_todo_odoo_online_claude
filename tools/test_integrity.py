...
def check_claude_md():
    ...
    out = [(код == 0, 'CLAUDE.md: на чинному тексті перевірка мовчить'),
           ('31 похідних чисел' in буф.getvalue(),
            'CLAUDE.md: перевірка справді дивиться на 31 число, а не на нуль')]
    d = tempfile.mkdtemp()
    справж = cm.MD
    for було, стало, підпис in [
        ('банк питань на 84 позиці', 'банк питань на 80 позиці', 'застаріле число'),
        ('фільтрує по всіх 548 рядках', 'фільтрує по всіх 538 рядках', 'стара сума рядків'),
    ]:
    ...
    # Той самий клас у другому документі. `docs/відкриті-питання.md` наступна
    # сесія читає одразу після CLAUDE.md, і 10.09.2026 у ньому було три
    # застарілі числа. Друга отрута окремо стереже переказ КП: строк гарантії
    # живе у фіксованому тексті пропозиції, а документ його повторює.
    відкриті = io.open('docs/відкриті-питання.md', encoding='utf-8').read()
    справж_o = cm.OPEN
    for було, стало, підпис in [
        ('**360 пунктів описано, 0 лишилося**', '**350 пунктів описано, 0 лишилося**',
         'застаріле число рецептів'),
        ('**15 робочих днів від демонстрації позиції**',
         '**10 робочих днів від демонстрації позиції**',
         'переказ гарантії розійшовся з КП'),
    ]:
        ...
        try:
            cm.OPEN = шо
            with _cc.redirect_stdout(буфo):
                кодo = cm.main()
        finally:
            cm.OPEN = справж_o
        out.append((кодo != 0 and 'відкриті питання' in буфo.getvalue(),
                    'відкриті питання: %s спіймано' % підпис))
    # Протиотрута: правило не мусить кричати на чинному тексті другого документа
    # (це вже перевірено першим рядком) — а мусить сказати «не знайдено», якщо
    # прив’язку переписали.
    шо2 = os.path.join(d, 'poison_open_gone.md')
    io.open(шо2, 'w', encoding='utf-8').write(
        відкриті.replace('**360 пунктів описано, 0 лишилося**', 'усі пункти описано', 1))
    буфo2 = io.StringIO()
    try:
        cm.OPEN = шо2
        with _cc.redirect_stdout(буфo2):
            кодo2 = cm.main()
    finally:
        cm.OPEN = справж_o
    out.append((кодo2 != 0 and 'не знайдено' in буфo2.getvalue(),
                'відкриті питання: зникле твердження назване, а не пропущене'))
    return out
...
def check_client_text():
    ...
    отрути = [
        ('Платформа: підписку Odoo Online (план Custom)',
         'Платформа: ПРРО й підписку Odoo Online (план Custom)',
...
def check_kp_terms():
    """Умови КП: посилання на пункт, гарантійне вікно і слово «половина».

    Три числа, які КЛІЄНТ читає як обіцянку, а живуть вони в прозі й похідні від
    інших місць: момент другого платежу — від НОМЕРА пункту приймання; «15 робочих
    днів» — сума двох інших строків того самого розділу; «половина» — від
    DIAG_CREDIT. Кожне може розійтися мовчки, і жодне не помітно на око: текст
    валідний, збірка проходить, у КП стоїть неправда.
    """
    html = io.open('artifacts/calculator.html', encoding='utf-8').read()
    out = [(bc.check_kp_refs(html) == [] and bc.check_kp_window(html) == []
            and bc.check_kp_half(html) == [],
            'на чинних умовах КП усі три правила мовчать')]

    # 1. Посилання на пункт приймання: зсунутий номер і переписане правило.
    отрути = [
        (bc.check_kp_refs, 'пункт 4 розділу «Приймання»', 'пункт 9 розділу «Приймання»',
         'в порожнє місце', 'посилання на неіснуючий пункт приймання'),
        (bc.check_kp_refs, "'4. Проєкт закрито, коли", "'4. Проєкт завершено, коли",
         'уже не про закриття', 'пункт перестав бути про закриття проєкту'),
        # 2. Гарантійне вікно як сума: розійтися можна з обох боків.
        (bc.check_kp_window, '5 робочих днів на зауваження', '7 робочих днів на зауваження',
         'виходить 17', 'збільшений строк на зауваження не зійшовся з вікном'),
        (bc.check_kp_window, '— 15 робочих днів від', '— 20 робочих днів від',
         'виходить 15', 'переписане вікно не зійшлося з власними числами'),
        (bc.check_kp_window, 'Немає зауважень за 5 робочих днів', 'Немає зауважень за 3 робочих днів',
         'названий по-різному', 'той самий строк названий двома числами'),
    ]
    for правило, було, стало, слово, підпис in отрути:
        assert було in html, було[:50]
        помилки = правило(html.replace(було, стало, 1))
        out.append((any(слово in e for e in помилки), 'КП-умови: %s спіймано' % підпис))

    # 3. Слово «половина» проти частки заліку — з обох боків.
    було_частка = bc.DIAG_CREDIT
    try:
        bc.DIAG_CREDIT = 0.3
        out.append((any('обіцяє' in e for e in bc.check_kp_half(html)),
                    'КП-умови: змінена частка залишила слово «половина» — спіймано'))
    finally:
        bc.DIAG_CREDIT = було_частка
    без = re.sub('половин', 'частин', html, flags=re.I)
    out.append((any('немає' in e for e in bc.check_kp_half(без)),
                'КП-умови: зникле слово «половина» при частці 0,5 — спіймано'))
    # Протиотрута: у нотатках журналу «половина пункту» зустрічається 15 разів і
    # до заліку не має стосунку — правило туди не дивиться.
    a, b = bc.межі_блоку(html, 'VERIFY')
    out.append((len(re.findall('половин', html[a:b], re.I)) > 5
                and bc.check_kp_half(html) == [],
                'КП-умови: «половина пункту» в журналі правило не чіпає'))
    return out


for okk, note in check_kp_terms():
    print(('  ok     ' if okk else '  ПРОВАЛ ') + note)
    res.append(okk)


def check_kp_words():
    """Дві знятих теми у фіксованому тексті КП: підписання і ПДВ за законодавством.

    Рішення 10.09.2026: нічого не підписуємо — тільки погодження; формулювання про
    ПДВ за законодавством України прибране. Обидві теми легко повертаються при
    наступній правці умов, і обидві помітні лише на око.
    """
    html = io.open('artifacts/calculator.html', encoding='utf-8').read()
    out = [(bc.check_kp_words(html) == [], 'на чинних умовах КП правило про слова мовчить')]
    for було, стало, слово, підпис in [
        ('Складаємо ми, погоджуємо разом', 'Складаємо ми, підписуємо разом',
         'обіцяють підписання', 'підписання повернулось в умови'),
        ("'Ціни в пропозиції — в євро, без ПДВ. Рахунок виставляється за курсом продажу євро на '",
         "'Ціни в євро без ПДВ — ПДВ додається за законодавством України.'",
         'законодавство про ПДВ', 'ПДВ за законодавством повернувся'),
    ]:
        assert було in html, було[:40]
        помилки = bc.check_kp_words(html.replace(було, стало, 1))
        out.append((any(слово in e for e in помилки), 'слова КП: %s спіймано' % підпис))
    # Дві протиотрути, обидві спіймані першим прогоном правила:
    # 1) «підписка» — той самий корінь, що «підпис», і власний розділ «Оплата»
    #    говорить про підписку тричі;
    # 2) «підписання у браузері за посиланням» — це ПРОДАНИЙ пункт прайсу
    #    (Odoo Sign), тому правило дивиться лише на фіксовану частину.
    фікс = bc.фіксований_текст(html)
    out.append(('підписку' in фікс and bc.check_kp_words(html) == [],
                'слова КП: «підписка» у власному тексті правило не чіпає'))
    import json as _js
    прайс = _js.load(io.open('data/price.json', encoding='utf-8'))
    є_sign = any('підписання' in t
                 for g in прайс['групи'] for it in g['items']
                 for t in it.get('inc', []) + [x for l in it['lv'] for x in l[4]])
    out.append((є_sign and 'підписання' not in фікс,
                'слова КП: пункт прайсу про Odoo Sign лишається (правило туди не дивиться)'))
    return out


for okk, note in check_kp_words():
    print(('  ok     ' if okk else '  ПРОВАЛ ') + note)
    res.append(okk)


def check_kp_wrap():
    """Ручні переноси у фіксованому тексті КП.

    Текст писався переносами під монospace-поле, а клієнт бачить його пропорційним
    шрифтом будь-якої ширини: переноси падають у середину речень, відступи
    продовження дають драбинку обірваних піврядків. Клас повертається при кожній
    правці умов, бо в редакторі довгий рядок незручний — і саме тому потрібне
    правило, а не звичка.
    """
    html = io.open('artifacts/calculator.html', encoding='utf-8').read()
    out = [(bc.check_kp_wrap(html) == [], 'у чинному тексті КП ручних переносів немає')]
    отрута = html.replace("    'ПРИЙМАННЯ',",
                          "    'ПРИЙМАННЯ',\n    '   продовження рядка руками',", 1)
    out.append((any('ручний перенос' in e for e in bc.check_kp_wrap(отрута)),
                'повернений ручний перенос спіймано'))
    return out


for okk, note in check_kp_wrap():
    print(('  ok     ' if okk else '  ПРОВАЛ ') + 'переноси КП: ' + note)
    res.append(okk)


def check_hidden_pages():
    """«Сховано» — це два твердження одночасно, і кожне ловить свою помилку.

    Сторінка «Повний перелік» схована від сейла рішенням 10.09.2026, але лишається
    в коді: її вмикають одним рядком. Тому перевіряється і те, що вона схована
    (інакше тихо повернулась користувачеві), і те, що розмітка на місці (інакше
    «сховати» стало «видалити», і повертати вже нічого).
    """
    html = io.open('artifacts/calculator.html', encoding='utf-8').read()
    out = [(bc.check_hidden_pages(html) == [], 'на чинній сторінці правило про сховані мовчить')]
    for було, стало, слово, підпис in [
        ("var СХОВАНІ = ['spec'];", 'var СХОВАНІ = [];',
         'знову видима', 'сторінка повернулась у вкладки'),
        ('id="page-spec"', 'id="page-old"',
         'не сховали, а видалили', 'розмітку сторінки видалили'),
        ("var СХОВАНІ = ['spec'];", 'var X = [];',
         'перестав бути одним місцем', 'перелік схованих зник із коду'),
    ]:
        assert було in html, було[:40]
        помилки = bc.check_hidden_pages(html.replace(було, стало, 1))
        out.append((any(слово in e for e in помилки), 'сховані: %s спіймано' % підпис))
    return out


for okk, note in check_hidden_pages():
    print(('  ok     ' if okk else '  ПРОВАЛ ') + note)
    res.append(okk)


def check_warranty_in_bounds():
    """Строк гарантії в «не входить» позиції — загальна умова не на своєму місці.

    Знайдено 10.09.2026 в «Базі»: рядок «не входить» називав гарантію на дефекти
    в робочих днях. Дві неправди в одному рядку — клієнт читає строк як обіцянку
    ЦІЄЇ позиції, а сам строк стає ще однією копією числа з «Приймання» і
    розходиться з ним мовчки.
    """
    d = json.load(io.open('data/price.json', encoding='utf-8'))
    e, _ = bc.check(d['групи'])
    out = [(not any('строк гарантії' in x for x in e),
            'на чинному прайсі правило про гарантію мовчить')]
    d['групи'][0]['items'][0].setdefault('bounds', []).append(
        'гарантія на дефекти діє 10 робочих днів після приймання')
    e2, _ = bc.check(d['групи'])
    out.append((any('строк гарантії' in x for x in e2),
                'загальна умова в «не входить» позиції спіймана'))
    return out


for okk, note in check_warranty_in_bounds():
    print(('  ok     ' if okk else '  ПРОВАЛ ') + note)
    res.append(okk)
...
def check_demo_brief():
    """Рівень повноти демо: чи справді він щось змінює і чи не мовчить бриф.

    Правило додане 15.09.2026 разом із самою опцією. Отрути перевіряють три
    способи зламати її тихо:

    1. менеджер не відповів — бриф мусить ЗУПИНИТИ роботу (код 2), а не
       поводитись як «максимальне» за замовчуванням і зібрати весь набір;
    2. рівень «зібране» пропускає крок без даних — і мусить НАЗВАТИ його в
       плані; мовчазний пропуск означає, що демо біднішає непомітно для
       менеджера, і побачить він це вже перед клієнтом;
    3. «показуємо» називає позицію поза набором — це описка, і вона мусить
       валити збірку, а не тихо звужувати демо до нуля.

    Протиотрута: на повному профілі нічого не пропускається — інакше правило
    кричало б на правильних даних.
    """
    import importlib.util, contextlib as _cc, json as _j, tempfile, os
    sp = importlib.util.spec_from_file_location('emitd', 'tools/emit_config_calls.py')
    ec = importlib.util.module_from_spec(sp); sp.loader.exec_module(ec)
    sp2 = importlib.util.spec_from_file_location('brief', 'tools/demo_brief.py')
    br = importlib.util.module_from_spec(sp2); sp2.loader.exec_module(br)

    d = tempfile.mkdtemp()
    проф = _j.load(io.open('data/client-profile-example.json', encoding='utf-8'))

    def записати(ім, п):
        ш = os.path.join(d, ім)
        io.open(ш, 'w', encoding='utf-8').write(_j.dumps(п, ensure_ascii=False))
        return ш

    out = []

    # ОТРУТА 1: профіль без відповіді менеджера
    без = dict(проф); без.pop('демо', None)
    буф = io.StringIO()
    with _cc.redirect_stdout(буф):
        код = br.main(['x', записати('без.json', без)])
    out.append((код == 2 and 'менеджер ще не відповів' in буф.getvalue(),
                'бриф зупиняє роботу, поки менеджер не вибрав повноту'))
    out.append(('## 1. Що показуємо на екрані' in буф.getvalue(),
                'бриф питає, які позиції справді будуть на екрані'))

    # ПРОТИОТРУТА: повний профіль — нічого не пропускається
    повн = dict(проф); повн['демо'] = {'повнота': 'зібране', 'показуємо': ['base']}
    буф = io.StringIO()
    with _cc.redirect_stdout(буф):
        код = ec.main(['x', записати('повн.json', повн)])
    out.append((код == 0 and 'Не збираємо — немає даних' not in буф.getvalue(),
                'на повному профілі рівень «зібране» нічого не пропускає'))

    # ОТРУТА 2: поля немає — крок пропускається І названий
    дірка = dict(повн); дірка.pop('логіни_продажів', None)
    буф = io.StringIO()
    with _cc.redirect_stdout(буф):
        код = ec.main(['x', записати('дірка.json', дірка)])
    текст = буф.getvalue()
    out.append((код == 0 and 'Не збираємо — немає даних' in текст
                and 'логіни_продажів' in текст,
                'пропущений крок названий у плані, а не зник мовчки'))

    # той самий профіль на «максимальне» — план не видається взагалі
    строго = dict(дірка); строго['демо'] = {'повнота': 'максимальне', 'показуємо': ['base']}
    буф = io.StringIO()
    with _cc.redirect_stdout(буф):
        код = ec.main(['x', записати('строго.json', строго)])
    out.append((код == 1, '«максимальне» не видає плану, поки профіль неповний'))

    # ОТРУТА 3: описка в «показуємо»
    описка = dict(повн); описка['демо'] = {'повнота': 'зібране', 'показуємо': ['basе']}
    try:
        with _cc.redirect_stdout(io.StringIO()):
            ec.main(['x', записати('описка.json', описка)])
        спіймано = False
    except SystemExit as e:
        спіймано = 'показуємо' in str(e)
    out.append((спіймано, 'позиція поза набором у «показуємо» валить збірку'))

    # рівень справді змінює обсяг роботи, а не лише підпис
    def кроків(п):
        б = io.StringIO()
        with _cc.redirect_stdout(б):
            ec.main(['x', записати('розмір.json', п)])
        р = [l for l in б.getvalue().splitlines() if l.startswith('Кроків')]
        return int(р[0].split()[3]) if р else -1
    вузько = dict(проф); вузько['демо'] = {'повнота': 'максимальне', 'показуємо': ['base']}
    широко = dict(проф); широко['демо'] = {'повнота': 'максимальне', 'показуємо': []}
    out.append((0 < кроків(вузько) < кроків(широко),
                'вибір «показуємо» справді скорочує план, а не тільки текст'))
    return out


for okk, note in check_demo_brief():
    print(('  ok     ' if okk else '  ПРОВАЛ ') + 'демо: ' + note)
    res.append(okk)
