...
#
# ЩЕ ОДНЕ, ЩО ЗʼЯСУВАЛОСЯ ПЕРЕВІРКОЮ 10.09.2026, і воно економить годину наступному:
# порожній результат ТОЧНОГО пошуку за назвою тут — норма, а не поломка. Перевірено
# на 18 кандидатах виду `ir.ui.view`: у базі не знайшлося ЖОДНОГО, а всі 18 артефактів
# існують. Дві причини, обидві системні:
#   1. у робочих місць назву з лапок носить ДІЯ (`ir.actions.act_window`), а не
#      подання — карта ж дає першою моделлю `ir.ui.view`, бо так написано в «де»;
#   2. назви подань мають суфікси («Воронка керівника (окреме подання під роль)»),
#      тому точний збіг не спрацьовує навіть у правильній моделі.
# Тому шукати треба ПІДРЯДКОМ і в парі «подання + дія», а «не знайдено» вважати
# приводом подивитися, ніколи — доказом відсутності.
#
# ЗАГАЛЬНЕ ПРАВИЛО, яке склалося після перевірки 60 кандидатів у чотирьох моделях:
# **назва з лапок майже завжди належить СУСІДНІЙ моделі, а не головній**. Головну
# дає карта (поле «де»), а в тексті цитують те, що видно людині:
#   робоче місце   → назва в ДІЇ, не в поданні
#   номенклатура   → «Товари / Кава» це complete_name КАТЕГОРІЇ, не товару
#   склад          → «Кава (Середнє)» це ВАРІАНТ (product.product), не шаблон
#   проєкти        → «Зібрати вимоги» це СТАДІЯ, не дія сервера
# З 60 перевірених кандидатів справжніх відсутностей — НУЛЬ; усі промахи були
# мимо моделі. Тому проба лишається генератором кандидатів, а не суддею: вона
# каже, ЩО подивитися, і навмисно не каже, чого немає.
...
NOT_NAMES = {
    ...
    'Клієнт', 'Постачальник',
    # Додано 10.09.2026: після переписування записів звірки проба почала брати
    # в кандидати НАШІ Ж ФОРМУЛЮВАННЯ з тексту («запис «перевірено раніше» був
    # хибним підтвердженням»). Це не назва запису, і шукати її в базі марно.
    'перевірено раніше', 'те саме, що вище', 'до виконання', 'Друк',
}
